...
class CMSIntegration {
    ...
    async loadFromFiles() {
        ...
        // Try to load from Netlify build output first
        try {
            const mainResponse = await fetch(`/content/data.json?t=${timestamp}`);
            if (mainResponse.ok) {
                this.data = await mainResponse.json();
                console.log('✅ Main data loaded from Netlify build');
            } else {
                throw new Error(`Main data file not found: ${mainResponse.status}`);
            }
        } catch (error) {
            console.log('Netlify build data not available, trying individual files...');
    ...
    async refreshData() {
        ...
        try {
            ...
            // Wait a moment for Netlify build to complete
            ...
            this.updateAboutPage();
            this.updateSiteImages();
            console.log('✅ CMS data refreshed successfully from Netlify build');
        } catch (error) {
...
window.addEventListener('message', (event) => {
    if (event.data && event.data.type === 'cms-save-success') {
        ...
        // Wait longer for Netlify build to complete
        setTimeout(() => {
            window.refreshCMS();
        }, 5000); // Tunggu 5 detik untuk Netlify build selesai
    }
});
...
if (window.CMS) {
    window.CMS.registerEventListener({
        'postSave': async ({ entry }) => {
            ...
            console.log('⏳ Waiting for Netlify build to complete...');
            // Wait longer for Netlify build and deploy
            setTimeout(() => {
                window.refreshCMS();
            }, 7000); // Tunggu 7 detik untuk build dan deploy selesai
        }
    });
}
